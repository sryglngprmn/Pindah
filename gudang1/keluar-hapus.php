...
$id = isset($_GET['id_keluar']) ? (int)$_GET['id_keluar'] : 0;

if ($id <= 0) {
    die("ID tidak valid: $id");
}

// Jalankan fungsi hapus_keluar() sebelum mengeluarkan output HTML
$status = hapus_keluar($id);
...
<head>
  ...
  <title>Hapus Barang keluar</title>
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function() {
      Swal.fire({
        ...
        text: "<?php echo ($status > 0) ? 'Barang keluar berhasil dihapus' : 'Barang keluar gagal dihapus'; ?>",
