...
$id = isset($_GET['id_masuk']) ? (int)$_GET['id_masuk'] : 0;

if ($id <= 0) {
    die("ID tidak valid: $id");
}

// Jalankan fungsi hapus_masuk() sebelum mengeluarkan output HTML
$status = hapus_masuk($id);
...
<head>
  ...
  <title>Hapus Barang Masuk</title>
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function() {
      Swal.fire({
        ...
        text: "<?php echo ($status > 0) ? 'Barang masuk berhasil dihapus' : 'Barang masuk gagal dihapus'; ?>",
